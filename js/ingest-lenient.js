...
window.lenientTSVIngest = async function lenientTSVIngest(url) {
  ...
  for (let i = startIdx; i < lines.length; i++) {
    ...
    if (cells.length < 10) {
      ...
      // >10 fields — right anchor last two, left anchor first six, merge middle into quote_text
      ...
      const mergedQuote = middle.join(" ").trim();
      cells = [...prefix, mergedQuote, "", ...suffix]; // tweet_text left empty
      note = `row ${i + 1} had ${cells.length} fields after merge; combined ${middle.length} middle fields into quote_text`;
    }
